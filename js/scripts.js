'use strict';

const toggleHidden = (...fields) => {
  fields.forEach((field) => {
    if (field.hidden === true) {
      field.hidden = false;
    } else {
      field.hidden = true;
    }
  });
};

const labelHidden = (form) => {
  form.addEventListener('focusout', (evt) => {
    const field = evt.target;
    const label = field.nextElementSibling;

    if (field.tagName === 'INPUT' && field.value && label) {
      label.hidden = true;
    } else if (label) {
      label.hidden = false;
    }
  });
};

const toggleDelivery = (elem) => {
  const delivery = elem.querySelector('.js-radio');
  const deliveryYes = elem.querySelector('.shop-page__delivery--yes');
  const deliveryNo = elem.querySelector('.shop-page__delivery--no');
  const fields = deliveryYes.querySelectorAll('.custom-form__input');

  delivery.addEventListener('change', (evt) => {
    if (evt.target.id === 'dev-no') {
      fields.forEach((inp) => {
        if (inp.required === true) {
          inp.required = false;
        }
      });

      toggleHidden(deliveryYes, deliveryNo);

      deliveryNo.classList.add('fade');
      setTimeout(() => {
        deliveryNo.classList.remove('fade');
      }, 1000);
    } else {
      fields.forEach((inp) => {
        if (inp.required === false) {
          inp.required = true;
        }
      });

      toggleHidden(deliveryYes, deliveryNo);

      deliveryYes.classList.add('fade');
      setTimeout(() => {
        deliveryYes.classList.remove('fade');
      }, 1000);
    }
  });
};

const filterWrapper = document.querySelector('.filter__list');
if (filterWrapper) {
  filterWrapper.addEventListener('click', (evt) => {
    const filterList = filterWrapper.querySelectorAll('.filter__list-item');

    filterList.forEach((filter) => {
      if (filter.classList.contains('active')) {
        filter.classList.remove('active');
      }
    });

    const filter = evt.target;

    filter.classList.add('active');
  });
}

const shopList = document.querySelector('.shop__list');
if (shopList) {
  shopList.addEventListener('click', (evt) => {
    const prod = evt.path || (evt.composedPath && evt.composedPath());

    if (prod.some((pathItem) => pathItem.classList && pathItem.classList.contains('shop__item'))) {
      const shopOrder = document.querySelector('.shop-page__order');

      toggleHidden(document.querySelector('.intro'), document.querySelector('.shop'), shopOrder);

      window.scroll(0, 0);

      shopOrder.classList.add('fade');
      setTimeout(() => shopOrder.classList.remove('fade'), 1000);

      const form = shopOrder.querySelector('.custom-form');
      labelHidden(form);
      toggleDelivery(shopOrder);
      // Находим нужный товар
      const selectProd = prod[0];
      // Получаем ID выбраного товара
      const selectProdId = selectProd.getAttribute('tabindex');
      document.querySelector('.product-item__field--id').textContent = selectProdId;
      document.querySelector('.custom-form__input--product_id').value = selectProdId;
      // Получаем Имя товара
      document.querySelector('.product-item__name').textContent = selectProd.querySelector(
        '.product__name'
      ).textContent;
      // Получаем Цену товара
      let productPrice = selectProd.querySelector('.product__price').textContent;
      const minSumFreeShip = document.querySelector('.span-del-minsum').textContent;
      document.querySelector('.product-item__field--price').textContent = productPrice;
      // Считаем итоговую стоимость
      productPrice = +productPrice.split(' ')[0];
      let deliveryPrice = document.querySelector('.span-del-price');

      if (productPrice < minSumFreeShip) {
        deliveryPrice = +deliveryPrice.textContent;
        productPrice = productPrice + deliveryPrice;
      } else {
        deliveryPrice.textContent = 0;
      }

      document.querySelector('.del-all-price').textContent = productPrice;
      // Получаем Акции товаров, если они есть
      if (selectProd.querySelector('.product__promo')) {
        document.querySelector('.product-item__field--promo').textContent = selectProd.querySelector(
          '.product__promo'
        ).textContent;
      }

      const buttonOrder = shopOrder.querySelector('.button');
      const popupEnd = document.querySelector('.shop-page__popup-end');
      const popupEndTitle = document.querySelector('.shop-page__end-title');

      buttonOrder.addEventListener('click', (evt) => {
        form.noValidate = true;

        const inputs = Array.from(shopOrder.querySelectorAll('[required]'));

        inputs.forEach((inp) => {
          if (!!inp.value) {
            if (inp.classList.contains('custom-form__input--error')) {
              inp.classList.remove('custom-form__input--error');
            }
          } else {
            inp.classList.add('custom-form__input--error');
          }
        });

        if (inputs.every((inp) => !!inp.value)) {
          evt.preventDefault();

          // Формируем массив с файлами для отправки на сервер
          const formData = new FormData(form);

          // Отправка на сервер через асинхронный запрос
          fetch('/modules/order/add.php', {
            method: 'POST',
            body: formData,
          })
            .then(() => {
              form.reset();
              form.noValidate = false;
              popupEndTitle.textContent = 'Спасибо за заказ!';
            })
            .catch((error) => {
              popupEndTitle.textContent = 'Заказ не отправлен!';
              console.log(error);
            })
            .finally(() => {
              console.log('Успешная отправка!');
            });

          toggleHidden(shopOrder, popupEnd);

          popupEnd.classList.add('fade');
          setTimeout(() => popupEnd.classList.remove('fade'), 1000);

          window.scroll(0, 0);

          const buttonEnd = popupEnd.querySelector('.button');

          buttonEnd.addEventListener('click', () => {
            popupEnd.classList.add('fade-reverse');

            setTimeout(() => {
              popupEnd.classList.remove('fade-reverse');

              toggleHidden(popupEnd, document.querySelector('.intro'), document.querySelector('.shop'));
            }, 1000);
          });
        } else {
          window.scroll(0, 0);
          evt.preventDefault();
        }
      });
    }
  });
}

const pageOrderList = document.querySelector('.page-order__list');
if (pageOrderList) {
  pageOrderList.addEventListener('click', (evt) => {
    if (evt.target.classList && evt.target.classList.contains('order-item__toggle')) {
      var path = evt.path || (evt.composedPath && evt.composedPath());
      Array.from(path).forEach((element) => {
        if (element.classList && element.classList.contains('page-order__item')) {
          element.classList.toggle('order-item--active');
        }
      });

      evt.target.classList.toggle('order-item__toggle--active');
    }

    if (evt.target.classList && evt.target.classList.contains('order-item__btn')) {
      const status = evt.target.previousElementSibling;
      let orderIdStatus = evt.target.parentElement.parentElement.parentElement.querySelector('.order-item__info--id')
        .textContent;
      const formDataStatus = new FormData();
      formDataStatus.append('id', orderIdStatus);
      if (status.classList && status.classList.contains('order-item__info--no')) {
        status.textContent = 'Выполнено';
        formDataStatus.append('processed', '1');
      } else {
        status.textContent = 'Не выполнено';
        formDataStatus.append('processed', '0');
      }

      // Отправляем асинхронный запрос
      fetch('/admin/modules/changeStatus.php', {
        method: 'POST',
        body: formDataStatus,
      })
        .then(() => console.log('Отправлен запрос в БД'))
        .catch((error) => console.log(error))
        .finally(() => console.log('Успешная отправка запроса!'));

      status.classList.toggle('order-item__info--no');
      status.classList.toggle('order-item__info--yes');
    }
  });
}

const checkList = (list, btn) => {
  if (list.children.length === 1) {
    btn.hidden = false;
  } else {
    btn.hidden = true;
  }
};
const addList = document.querySelector('.add-list');
if (addList) {
  const form = document.querySelector('.custom-form');
  labelHidden(form);

  const addButton = addList.querySelector('.add-list__item--add');
  const addInput = addList.querySelector('#product-photo');

  checkList(addList, addButton);

  // Получаем параметры GET
  let getParams = new URL(document.location).searchParams;
  if (typeof getParams.get('change-id') === 'string') {
    addInput.removeAttribute('required');
    addInput.setAttribute('disabled', 'disabled');
    document.querySelector('.add-list__item--active').addEventListener('click', (evt) => {
      addList.removeChild(evt.target);
      addInput.value = '';
      addInput.removeAttribute('disabled');
      addInput.setAttribute('required', 'required');
      checkList(addList, addButton);
    });
  }

  addInput.addEventListener('change', (evt) => {
    const template = document.createElement('LI');
    const img = document.createElement('IMG');

    template.className = 'add-list__item add-list__item--active';
    template.addEventListener('click', (evt) => {
      addList.removeChild(evt.target);
      addInput.value = '';
      checkList(addList, addButton);
    });

    const file = evt.target.files[0];
    const reader = new FileReader();

    reader.onload = (evt) => {
      img.src = evt.target.result;
      template.appendChild(img);
      addList.appendChild(template);
      checkList(addList, addButton);
    };

    reader.readAsDataURL(file);
  });

  const button = document.querySelector('.button');
  const popupEnd = document.querySelector('.page-add__popup-end');

  button.addEventListener('click', (evt) => {
    const inputs = Array.from(form.querySelectorAll('[required]'));
    console.log(inputs);

    inputs.forEach((inp) => {
      if (!!inp.value) {
        if (inp.classList.contains('custom-form__input--error')) {
          inp.classList.remove('custom-form__input--error');
        }
      } else {
        inp.classList.add('custom-form__input--error');
      }
    });

    if (inputs.every((inp) => !!inp.value)) {
      evt.preventDefault();
      // Формируем массив с файлами для отправки на сервер
      const formDataAdd = new FormData(form);

      // Отправка на сервер через асинхронный запрос
      fetch('/admin/modules/add.php', {
        method: 'POST',
        body: formDataAdd,
      })
        .then(() => {
          form.reset();
        })
        .catch((error) => {
          document.querySelector('.shop-page__end-title').textContent = 'Ошибка отправки, поробуйте снова';
        })
        .finally(() => {
          console.log('Успешная отправка!');
        });

      form.hidden = true;
      popupEnd.hidden = false;
    } else {
      window.scroll(0, 0);
      evt.preventDefault();
    }
  });
}

const productsList = document.querySelector('.page-products__list');
if (productsList) {
  productsList.addEventListener('click', (evt) => {
    const target = evt.target;
    if (target.classList && target.classList.contains('product-item__delete')) {
      productsList.removeChild(target.parentElement);

      let productId = target.parentElement.querySelector('.product-item__field--id').textContent;
      const formDataProduct = new FormData();
      formDataProduct.append('id', productId);
      formDataProduct.append('active', '0');
      // Отправляем асинхронный запрос
      fetch('/admin/modules/changeStatus.php', {
        method: 'POST',
        body: formDataProduct,
      })
        .then(() => console.log('Отправлен запрос в БД'))
        .catch((error) => console.log(error))
        .finally(() => console.log('Успешная отправка запроса!'));
    }
  });
}

let minPrice = 0;
let maxPrice = 0;

// jquery range maxmin
if (document.querySelector('.shop-page')) {
  // Получаем данные для слайдера из PHP
  minPrice = +document.querySelector('.range__input--min').value;
  maxPrice = +document.querySelector('.range__input--max').value;

  // Получаем параметры GET
  let getParams = new URL(document.location).searchParams;
  const minRangeValue = getParams.get('filter[price-min]') ?? minPrice;
  const maxRangeValue = getParams.get('filter[price-max]') ?? maxPrice;

  // Устанавливаем
  document.querySelector('.min-price').textContent = minRangeValue + ' руб.';
  document.querySelector('.max-price').textContent = maxRangeValue + ' руб.';

  $('.range__line').slider({
    min: minPrice,
    max: maxPrice,
    values: [minRangeValue, maxRangeValue],
    range: true,
    stop: function (event, ui) {
      $('.min-price').text($('.range__line').slider('values', 0) + ' руб.');
      $('.max-price').text($('.range__line').slider('values', 1) + ' руб.');
    },
    slide: function (event, ui) {
      $('.min-price').text($('.range__line').slider('values', 0) + ' руб.');
      $('.max-price').text($('.range__line').slider('values', 1) + ' руб.');
    },
  });

  const formFilter = document.querySelector('.shop__form');

  // Обработка отправки данных с формы
  formFilter.addEventListener('submit', function (e) {
    document.querySelector('.range__input--min').value = $('.range__line').slider('values', 0);
    document.querySelector('.range__input--max').value = $('.range__line').slider('values', 1);
  });

  // Обработка сортировки товаров
  // Функция сохранения атрибуты при отправке запроса
  function getSelectAtribute(select, getValue) {
    for (let index = 0; index < select.options.length; index++) {
      const element = select.options[index];
      if (element.value === getValue) {
        select.options[index].setAttribute('selected', true);
      }
    }
  }
  // Функция получение Сортировки и порядка товаров
  function getSortAndOrderProducts() {
    // Находим базовые селекторы
    const selectSort = document.querySelector('.select-sort');
    const selectOrder = document.querySelector('.select-order');
    const selectForm = document.querySelector('.shop__sorting');

    // Получаем данные GET
    let getParams = new URL(document.location).searchParams;
    const sortGetValue = getParams.get('sort') ?? '';
    const orderGetValue = getParams.get('order') ?? '';

    // Если был вабран ранее атрибут
    getSelectAtribute(selectSort, sortGetValue);
    getSelectAtribute(selectOrder, orderGetValue);

    // Проверяем заполненность селектов
    selectSort.addEventListener('change', function (e) {
      if (selectOrder.value) {
        selectForm.submit();
      }
    });

    selectOrder.addEventListener('change', function (e) {
      if (selectSort.value) {
        selectForm.submit();
      }
    });
  }

  getSortAndOrderProducts();
}
